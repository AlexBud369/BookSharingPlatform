...
import { MAX_EMAIL_LENGTH, MIN_PASSWORD_LENGTH } from '../constants';

const schema = yup.object({
  email: yup.string().required('EmailRequired').email('InvalidEmailFormat').max(MAX_EMAIL_LENGTH, 'EmailTooLong'),
  password: yup.string().required('PasswordRequired').min(MIN_PASSWORD_LENGTH, 'PasswordTooShort'),
});
...
export default function LoginForm() {
  ...
  const onSubmit = async (data: LoginDto) => {
    try {
      ...
      toast.error(t(error.response?.data?.errors?.[0] || 'InvalidCredentials'));
    }
  };
